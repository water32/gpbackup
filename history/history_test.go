...
var _ = Describe("backup/history tests", func() {
	...
	Describe("Sections tests", func() {
		...
		It("finds sections with Contains", func() {
			s = history.Predata | history.Data | history.Postdata
		...
		DescribeTable("ValidateBackup",
			...
			// Flag combinations
			Entry("empty string", []string{"--section", ""}, history.Empty, history.Empty, MatchError("No section flags provided")),
			Entry("invalid section", []string{"--section", "invalid"}, history.Empty, history.Empty, MatchError("Unrecognized section name: invalid")),
			...
			Entry("--data-only", []string{"--data-only"}, history.Data, history.Predata|history.Postdata, Not(HaveOccurred())),
		...
		DescribeTable("ValidateRestore",
		func(args []string, configSections history.Sections, expected types.GomegaMatcher) {
			Expect(flags.Parse(args)).To(Succeed())
			err = s.ValidateRestore(flags, &history.BackupConfig{Sections: configSections})
