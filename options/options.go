...
func getOidsFromRelationList(relationList []Relation) []string {
	oids := make([]string, len(relationList))
	for idx, rel := range relationList {
		oids[idx] = strconv.FormatUint(uint64(rel.Oid), 10)
	}
	return oids
}
