...
func PrintCreateDatabaseStatement(metadataFile *utils.FileWithByteCount, tocfile *toc.TOC, defaultDB Database, db Database, dbMetadata MetadataMap) {
	...
	tocfile.AddMetadataEntry("global", entry, start, metadataFile.ByteCount, 0)
	PrintObjectMetadata(metadataFile, tocfile, dbMetadata[db.GetUniqueID()], db, "", 0)
}
...
func PrintCreateResourceQueueStatements(metadataFile *utils.FileWithByteCount, tocfile *toc.TOC, resQueues []ResourceQueue, resQueueMetadata MetadataMap) {
	for _, resQueue := range resQueues {
		...
		PrintObjectMetadata(metadataFile, tocfile, resQueueMetadata[resQueue.GetUniqueID()], resQueue, "", 0)
	}
}
...
func PrintCreateResourceGroupStatements(metadataFile *utils.FileWithByteCount, toc *toc.TOC, resGroups []ResourceGroup, resGroupMetadata MetadataMap) {
	for _, resGroup := range resGroups {
		...
		if resGroup.Name == "default_group" || resGroup.Name == "admin_group" {
			...
			toc.AddMetadataEntry(section, entry, start, metadataFile.ByteCount, 0)
			PrintObjectMetadata(metadataFile, toc, resGroupMetadata[resGroup.GetUniqueID()], resGroup, "", 0)
		} else {
			start = metadataFile.ByteCount
			attributes := make([]string, 0)
...
ALTER ROLE %s WITH %s;`, role.Name, role.Name, strings.Join(attrs, " "))
		...
		PrintObjectMetadata(metadataFile, toc, roleMetadata[role.GetUniqueID()], role, "", 0)
	}
}
...
func PrintCreateTablespaceStatements(metadataFile *utils.FileWithByteCount, toc *toc.TOC, tablespaces []Tablespace, tablespaceMetadata MetadataMap) {
	for _, tablespace := range tablespaces {
		...
		PrintObjectMetadata(metadataFile, toc, tablespaceMetadata[tablespace.GetUniqueID()], tablespace, "", 0)
	}
}
